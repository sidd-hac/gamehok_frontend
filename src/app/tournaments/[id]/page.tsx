  ...
  if (!tournament) {
    return (
      <div className="flex items-center justify-center h-screen text-white">
        <p className="text-2xl font-semibold text-gray-400">Tournament Not Found</p>
      </div>
    );
  }
        ...
        <div className="flex justify-between items-center border-b border-gray-700 pb-3">
          ...
          <span
            className={`px-3 py-1 rounded-full text-sm font-semibold ${
              tournament.status === "Upcoming" ? "bg-blue-600" : "bg-green-600"
            }`}
          >
            {tournament.status}
          </span>
        ...
        <div className="grid grid-cols-2 gap-6 mt-4">
          <div>
            ...
            <p className="text-gray-300">
              🏆 Prize Pool: <span className="text-yellow-400 font-semibold">${tournament.prizePool.toLocaleString()}</span>
            </p>
          </div>
            ...
            <div className="bg-gray-700 p-3 rounded-lg">
              {tournament.prizeBreakdown.map((prize, index) => (
                ...
                >
                  ...
                  <span className="text-yellow-400 font-semibold">${prize.amount.toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
